...
class PCMRecordingSink:  # noqa: D101
...
def create_bot() -> commands.Bot:
    ...
    async def stop(interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            logger.debug("stop_listening failed or was already stopped", exc_info=True)
        ...
        byte_count = session.sink.write_wav(output_path)

        await interaction.followup.send(
            f"✅ Recording stopped from **{session.channel_name}**.\n"
            f"Saved `{output_path}` ({byte_count} bytes).",
